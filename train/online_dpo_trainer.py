...
def label_batchify(data: Iterable[T], batch_size: int) -> Iterable[List[T]]:
    assert batch_size > 0
    batch = []
    for item in data:
        # Yield next batch
        if len(batch) == batch_size:
            yield batch
            batch = []
        batch.append(f"{item['label']}")
    # Yield last un-filled batch
    if len(batch) != 0:
        yield batch

def ret_selected_ls(final_ls, all_rank_stats, mode, prompts, f, labels, rand_sft_ls=None):
    # estimate mean reward of each sample
    ucb_ls, lcb_ls = [], []
    for sen_index in range(len(all_rank_stats[0])):
        cur_ucb_ls, cur_lcb_ls, cur_reward_ls, cur_std_ls = [], [], [], []
        for sample_index in range(len(all_rank_stats[0][sen_index])):
            ...
            cur_reward_ls += [str(ele_mean_reward)]
            cur_std_ls += [str(ele_std_reward)]
            cur_ucb_ls += [ele_mean_reward+config.gamma * ele_std_reward]
            cur_lcb_ls += [ele_mean_reward-config.gamma * ele_std_reward]
        ucb_ls+=[cur_ucb_ls]
        lcb_ls+=[cur_lcb_ls]
        f.write(' '.join(cur_reward_ls)+'\t'+' '.join(cur_std_ls)+'\n')

    final_prompts, first_ls, sec_ls = [], [], []
    # randomly determine first and sec location in annotation
    ...
    if mode == "ucb_sec_ucb": # first and sec are definately different
        for sen_ls, cur_sen_ls, prompt in zip(ucb_ls, final_ls, prompts):
            if len(cur_sen_ls)>1:
                ...
                if rand_index == 0:
                    ...
                    first_ls+=[cur_sen_ls[sec_index]]
                    sec_ls+=[cur_sen_ls[first_index]]
        ...
        for sen_index, (sen_ls, cur_sen_ls, prompt) in enumerate(zip(ucb_ls, final_ls, prompts)):
            if len(cur_sen_ls)>1:
                sort_ls=sorted([[score, index] for index, score in enumerate(sen_ls)])
                # select first sentence based on UCB
                first_index=sort_ls[-1][1]
                if config.run_sample_strategy=="merged_sft":
                    sec_sen=random.sample(rand_sft_ls[sen_index], 1)[0]
                    # only if sft sentence and ucb sentence are differed
                    if cur_sen_ls[first_index] != sec_sen:
                        if rand_index == 0:
                            first_ls+=[cur_sen_ls[first_index]]
                            sec_ls+=[sec_sen]
                        else:
                    ...
                    # print(f"sentence index {sen_index}: ({first_index}, {sec_index})")
                    if rand_index == 0:
                        first_ls+=[cur_sen_ls[first_index]]
                        sec_ls+=[cur_sen_ls[sec_index]]
                    else:
        ...
        for sen_ls, cur_sen_ls, prompt, label in zip(ucb_ls, final_ls, prompts, labels):
            if len(cur_sen_ls)>1:
                sort_ls=sorted([[score, index] for index, score in enumerate(sen_ls)])
                # select first sentence based on UCB
                first_index=sort_ls[-1][1]
...
def ret_rank_dict(prompts, tokenizer, model, num_ret, mode, num_lora, sampling_strategy):
    model.eval()
    final_ls = []
    if mode != "rand_rand":
    ...
    with torch.no_grad():
        for prompt in prompts:
            inputs = tokenizer([prompt], return_tensors="pt", padding=True, truncation=True, max_length=config.max_prompt_length).to('cuda') 
            # make sure the inference adapter name is correct
            summary_ls = []
            start_time=time.time()
            # select one sample explicitly from sft model
            if config.run_sample_strategy=="merged_sft":
                rand_sft_ls = []
                ...
                if sampling_strategy == "top-p":
                    if config.rank_strategy == "reward":
                        ...
                        output = model.generate(inputs=inputs.input_ids, max_new_tokens=config.max_new_token, do_sample=True, temperature=1.0, top_k=0, top_p=0.95, 
                                                num_return_sequences=2, return_dict_in_generate=True, output_scores=True)
                    else:
                    ...
                    exit(1)
                rand_sft_ls += list(set(tokenizer.batch_decode(output[:, inputs.input_ids.shape[1]:], skip_special_tokens=True)))

            if config.run_sample_strategy=="merged" or config.run_sample_strategy=="sft" or config.run_sample_strategy=="merged_sft" or config.run_sample_strategy=="per_interval_merged":
                ...
                for _ in range(int(num_ret/config.gpu_takes_n)):
                    if sampling_strategy == "top-p":
                        if config.rank_strategy == "reward":
                            output = model.generate(inputs=inputs.input_ids, max_new_tokens=config.max_new_token, do_sample=True, temperature=1.0, top_k=0, top_p=0.95, 
                                                    num_return_sequences=config.gpu_takes_n)
                        elif config.rank_strategy == "logits":
                            output = model.generate(inputs=inputs.input_ids, max_new_tokens=config.max_new_token, do_sample=True, temperature=1.0, top_k=0, top_p=0.95, 
                                                    num_return_sequences=config.gpu_takes_n, return_dict_in_generate=True, output_scores=True)
                        else:
                            print("Your rank strategy is not supported!")
                            exit(1)
                    elif sampling_strategy == "ancestral":
            ...
            if mode != "rand_rand":
                # get rewards for our response
                start_time=time.time()
                selected_data={'prompt': [prompt]*int(len(summary_ls)/2), "chosen": summary_ls[:int(len(summary_ls)/2)], "rejected": summary_ls[int(len(summary_ls)/2):int(len(summary_ls)/2)*2]}
                if config.train_baseline_model_mode == 'merge_on_interval':
                    ref_model_index = f"cur_{config.dpo_inference_name}"
                elif config.train_baseline_model_mode == 'sft':
                    ref_model_index = config.sft_adapter_name
                else:
                    print("We dont support other ref models")
                    exit(1)
                reference_chosen_logps_ls, reference_rejected_logps_ls = compute_ref_reward_from_models(selected_data, config.data_collator, model, ref_model_index)
                print("Ref logits time: ", time.time()-start_time)
                # compute rewards for each response
                start_time=time.time()
                for model_index in range(0, num_lora):
                    # print(f"Model {model_index} logits compute:")
                    rewards_ls_1, rewards_ls_2 = compute_reward_from_models(selected_data, config.data_collator, model, f"model_{model_index}", 
                                                                ref_model_index, reference_chosen_logps_ls, reference_rejected_logps_ls)
                    rewards_ls=rewards_ls_1+rewards_ls_2
                    sort_ls = [sorted(rewards_ls).index(x) for x in rewards_ls]
                    all_rank_stats[model_index] += [sort_ls]
                print("Policy logits time: ", time.time()-start_time)
            print("Current sample size: ", len(summary_ls))
            final_ls+=[summary_ls]
            if config.run_sample_strategy=="merged_sft":
                final_sft_ls+=[rand_sft_ls]

    if config.run_sample_strategy=="merged_sft":
        if mode == "rand_rand":
            return final_ls, None, final_sft_ls
        else:
            return final_ls, all_rank_stats, final_sft_ls
    else:
        if mode == "rand_rand":
            return final_ls, None
        else:
            return final_ls, all_rank_stats

def inference(prompts, tokenizer, model, num_ret, mode, num_lora, sampling_strategy, labels):
    if config.run_sample_strategy=="merged_sft":
        final_ls, all_rank_stats, final_sft_ls = ret_rank_dict(prompts, tokenizer, model, num_ret, mode, num_lora, sampling_strategy)
    else:
        final_ls, all_rank_stats = ret_rank_dict(prompts, tokenizer, model, num_ret, mode, num_lora, sampling_strategy)
    
    first_ls, sec_ls, final_prompt_ls = [], [], []
    if mode == "rand_rand":
        # perform random selections of two samples
        if config.run_sample_strategy=="merged_sft":
            for ele, sft_ele, prompt in zip(final_ls, final_sft_ls, prompts):
                if len(ele) > 0 and len(sft_ele) > 0:
                    temp_ls_1 = random.sample(ele, 1)
                    # make sure one random selection is from sft model
                    temp_ls_2 = random.sample(sft_ele, 1)
                    if temp_ls_1[0] != temp_ls_2[0]:
                        first_ls += [temp_ls_1[0]]
                        sec_ls += [temp_ls_2[0]]
    ...
    elif mode == "upper_bound" or mode == "lower_bound":
        num_cor, total = 0, 0
        for sen_index, (ele, prompt) in enumerate(zip(final_ls, prompts)):
            cur_numpy = 0
            proposed_ls = []
            undesired_ls = []
            if len(ele) > 1:
                scores_ls = compute_gt_reward([prompt]*len(ele), ele)
                for i in range(num_lora):
                    cur_numpy+=np.array(all_rank_stats[i][sen_index])
                model_reward_ls=(cur_numpy/num_lora).tolist()
                cur_final_ls = list(zip(list(range(num_ret)), scores_ls, model_reward_ls))
                rank_pairs = list(combinations(cur_final_ls, 2))
                for rank_pair in rank_pairs:
                    if mode == "upper_bound":
                        if rank_pair[0][1]>rank_pair[1][1] and rank_pair[0][2]<rank_pair[1][2]:
                            proposed_ls += [[rank_pair[0][0], rank_pair[1][0]]]
                        elif rank_pair[0][1]<rank_pair[1][1] and rank_pair[0][2]>rank_pair[1][2]:
                            proposed_ls += [[rank_pair[0][0], rank_pair[1][0]]]
                        else:
                            undesired_ls += [[rank_pair[0][0], rank_pair[1][0]]]
                    else:
                        if rank_pair[0][1]>rank_pair[1][1] and rank_pair[0][2]>rank_pair[1][2]:
                            proposed_ls += [[rank_pair[0][0], rank_pair[1][0]]]
                        elif rank_pair[0][1]<rank_pair[1][1] and rank_pair[0][2]<rank_pair[1][2]:
                ...
                if len(proposed_ls)>0:
                    selected_pair = random.sample(proposed_ls, 1)[0]   
                    # for upper bound, proposed list contains all current mis-ranking pairs and they need to be correct
                    if mode == "upper_bound":
                        num_cor+=1
                else:
                    selected_pair = random.sample(undesired_ls, 1)[0]
                    # for lower bound, proposed list contains all current correct ranking pairs. So undesired list is right selection 
                    if mode == "lower_bound":
                        num_cor+=1
                total+=1
                first_ls += [ele[selected_pair[0]]]
                sec_ls += [ele[selected_pair[1]]]
                final_prompt_ls += [prompt]
            
        if config.logging_selection:
            config.per_batch_acc = num_cor/total
            config.all_rank_acc += config.per_batch_acc
    else:
        with open(f"{config.prefix}/reward_std_{mode}_{config.seed}_{config.gamma}_reward.txt", 'a') as f:
            if config.run_sample_strategy=="merged_sft":
                final_prompt_ls, first_ls, sec_ls = ret_selected_ls(final_ls, all_rank_stats, mode, prompts, f, labels, final_sft_ls)
            else:
                final_prompt_ls, first_ls, sec_ls = ret_selected_ls(final_ls, all_rank_stats, mode, prompts, f, labels)
...
def training_step(selected_data, data_collator, model, cur_acc_steps, optimizer_ls, lr_scheduler_ls, num_lora):
    ...
    with torch.no_grad():
        ...
        for padded_batch in data_loader:
            # switch to reference adapter
            (reference_chosen_logps, reference_rejected_logps, _, _,) = concatenated_forward(model, padded_batch)
            config.chosen_ref_logp += reference_chosen_logps.item() / cur_acc_steps
            config.rejected_ref_logp += reference_rejected_logps.item() / cur_acc_steps
    ...
    for model_index in range(num_lora):
        ...
        for cur_index, padded_batch in enumerate(data_loader):
            ...
            config.tr_loss_ls[model_index]+=losses.item() / cur_acc_steps
            loss = losses / cur_acc_steps
            loss.backward()
            # stepwise rewards
            config.reward_acc_ls[model_index] += (chosen_rewards > rejected_rewards).float() / cur_acc_steps
            # stepwise logps for policy chosen, policy rejected, ref chosen and ref rejected
            config.chosen_policy_logp[model_index] += policy_chosen_logps.item() / cur_acc_steps
            config.rejected_policy_logp[model_index] += policy_rejected_logps.item() / cur_acc_steps
            config.chosen_rewards[model_index] += chosen_rewards.item() / cur_acc_steps
            config.rejected_rewards[model_index] += rejected_rewards.item() / cur_acc_steps
            config.rewards_margins[model_index] += (chosen_rewards - rejected_rewards).item() / cur_acc_steps
...
def eval_over_steps(model, rank_model, rank_tok, tokenizer):
    model.eval()
    model.set_adapter(config.dpo_inference_name)
    ...
    with torch.no_grad():
        ...
        for prompts in prompt_batchify(eval_dataset, 8):
        ...
        eval_batch_size=16
        for response_ls, eval_data_batch in zip(batchify(data, eval_batch_size), batchify(eval_dataset, eval_batch_size)):
            prompt_batch=[ele['prompt'] for ele in eval_data_batch]
            inputs = rank_tok(prompt_batch, [ele['label'] for ele in eval_data_batch], return_tensors='pt', padding=True, truncation=True, max_length=640).to('cuda')
            ref_score_ls = rank_model(**inputs).logits.cpu().detach().tolist()

            inputs = rank_tok(prompt_batch, response_ls, return_tensors='pt', padding=True, truncation=True, max_length=640).to('cuda')
...
@click.option('-offline_enable', type=bool, default=False)
...
def main(mode, seed, api_source, start_index, end_index, num_lora, num_ret, prefix, gpu_takes_n, per_epoch_save, lr, 
         ...
         data_capacity, replay_step, offline_enable, shuffle_data, rank_strategy, on_policy_rand, run_sample_strategy, 
    ...
    config.per_epoch_step=int(config.train_size/(config.batch_size*config.acc_steps))
    config.lr=lr
    config.seed=seed
    config.gamma=gamma
    ...
    if resume:
        model=set_up_single_lora(model, f"/data/user_data/xixu/wendaxu/may_14/epoch_1_{mode}_{seed}_may_10_5e-05_0.0_resume_False_replay_{replay_step}_{rank_strategy}_on_policy_{on_policy_rand}/model_0", f"model_0")
        model.load_adapter(f"/data/user_data/xixu/wendaxu/may_14/epoch_1_{mode}_{seed}_may_10_5e-05_0.0_resume_False_replay_{replay_step}_{rank_strategy}_on_policy_{on_policy_rand}/{config.sft_adapter_name}", adapter_name=config.sft_adapter_name)
    else:
        ...
        if task_type == "tldr":
            ...
            model=set_up_single_lora(model, "/data/user_data/xixu/wendaxu/sft_harm_final/model_0/checkpoint-52", f"model_0")
            # load sft lora weights for inference
            sft_lora_addr_ls = [f"/data/user_data/xixu/wendaxu/sft_harm_final/model_{i}/checkpoint-52" for i in range(num_lora)]
        else:
    ...
    wandb.init(project='DPO_Master', name=f"{mode}_{seed}_{rank_strategy}_{num_ret}_sample_{run_sample_strategy}_{sampling_update_step}_{train_baseline_model_mode}_lora_{num_lora}_alpha_{config.alpha}_{task_type}", config=\
    {
        "seed": seed,
        "mode": mode,
    ...
    with tqdm(total=config.max_step) as pbar:
        for epoch_index in range(config.num_epoch):
            model.zero_grad()
            # if offline enabled, we perform offline DPO training
            if offline_enable:
                print("Starts offline data training")
                p_ls, c_ls, r_ls = [], [], []
                for i in range(config.per_epoch_step):
                    cur_data = json.load(open(f'{prefix}/offline_train_data_{seed}/epoch_0/{i}_rank.json'))
                ...
                for cur_p_ls, cur_c_ls, cur_r_ls in zip(batchify(p_ls, config.acc_steps*config.step_per_feedback), batchify(c_ls, config.acc_steps*config.step_per_feedback), batchify(r_ls, config.acc_steps*config.step_per_feedback)):
                    selected_data = {} 
                    ...
                    if logging_selection:
                        ...
                        config.all_rank_acc += sum(rate_ls)/len(rate_ls) # config.per_batch_acc

                    # evaluate on the holdout set
                    if config.global_step_ls[0] % eval_step == 0:    
                        model = merge_lora_for_inference(model, num_lora, config.dpo_inference_name)
                        config.reward_model=config.reward_model.to('cuda')
                        win_rate, reward_diff = eval_over_steps(model, config.reward_model, config.reward_tok, config.tokenizer)
                        # delete adapter atfer merging
                    ...
                    training_step(selected_data, data_collator, model, cur_acc_steps, optimizer_ls, lr_scheduler_ls, num_lora) # please use num_lora in the end

                    for i in range(num_lora):
                        if config.global_step_ls[i] % save_step == 0:
                            model.save_pretrained(f'{prefix}/epoch_{epoch_index}_{mode}_{seed}_may_10_{config.lr}_{config.gamma}_resume_{resume}_{config.global_step_ls[i]}_replay_{replay_step}_offline_{offline_enable}_shuffle_{shuffle_data}_{rank_strategy}_ret_{num_ret}_sample_{run_sample_strategy}_{train_baseline_model_mode}')
                            # save optimizer and lr scheduler
                            torch.save(optimizer_ls[i].state_dict(), f'{prefix}/epoch_{epoch_index}_{mode}_{seed}_may_10_{config.lr}_{config.gamma}_resume_{resume}_{config.global_step_ls[i]}_replay_{replay_step}_offline_{offline_enable}_shuffle_{shuffle_data}_{rank_strategy}_ret_{num_ret}_sample_{run_sample_strategy}_{train_baseline_model_mode}/optimizer_{i}.pt')
                            torch.save(lr_scheduler_ls[i].state_dict(), f'{prefix}/epoch_{epoch_index}_{mode}_{seed}_may_10_{config.lr}_{config.gamma}_resume_{resume}_{config.global_step_ls[i]}_replay_{replay_step}_offline_{offline_enable}_shuffle_{shuffle_data}_{rank_strategy}_ret_{num_ret}_sample_{run_sample_strategy}_{train_baseline_model_mode}/scheduler_{i}.pt')
                    pbar.update(1)
            else:
                # load in dataset
                ...
                for step, (prompts, labels) in enumerate(zip(prompt_batchify(sampling_prompt_dataset, config.acc_steps*config.step_per_feedback), label_batchify(sampling_prompt_dataset, config.acc_steps*config.step_per_feedback))):
                    ...
                    prompts, first_ls, sec_ls = inference(prompts, config.tokenizer, model, num_ret, mode, num_lora, sampling_strategy, labels)
                    ...
                    if not os.path.exists(f'{prefix}/online_train_data_{seed}_{mode}_{config.gamma}_reward_replay_{replay_step}_{rank_strategy}_on_policy_{on_policy_rand}_ret_{num_ret}_sample_{run_sample_strategy}_{sampling_update_step}_{train_baseline_model_mode}'):
                        os.mkdir(f'{prefix}/online_train_data_{seed}_{mode}_{config.gamma}_reward_replay_{replay_step}_{rank_strategy}_on_policy_{on_policy_rand}_ret_{num_ret}_sample_{run_sample_strategy}_{sampling_update_step}_{train_baseline_model_mode}')
                    if not os.path.exists(f'{prefix}/online_train_data_{seed}_{mode}_{config.gamma}_reward_replay_{replay_step}_{rank_strategy}_on_policy_{on_policy_rand}_ret_{num_ret}_sample_{run_sample_strategy}_{sampling_update_step}_{train_baseline_model_mode}/epoch_{epoch_index}'):
                        os.mkdir(f'{prefix}/online_train_data_{seed}_{mode}_{config.gamma}_reward_replay_{replay_step}_{rank_strategy}_on_policy_{on_policy_rand}_ret_{num_ret}_sample_{run_sample_strategy}_{sampling_update_step}_{train_baseline_model_mode}/epoch_{epoch_index}')

                    with open(f'{prefix}/online_train_data_{seed}_{mode}_{config.gamma}_reward_replay_{replay_step}_{rank_strategy}_on_policy_{on_policy_rand}_ret_{num_ret}_sample_{run_sample_strategy}_{sampling_update_step}_{train_baseline_model_mode}/epoch_{epoch_index}/{step}_rank.json', 'w') as f:
                    ...
                    for _ in range(replay_step):
                        selected_data = replay_data_buffer(f'{prefix}/online_train_data_{seed}_{mode}_{config.gamma}_reward_replay_{replay_step}_{rank_strategy}_on_policy_{on_policy_rand}_ret_{num_ret}_sample_{run_sample_strategy}_{sampling_update_step}_{train_baseline_model_mode}', data_capacity, num_lora, step, epoch_index)
                        training_step(selected_data, data_collator, model, cur_acc_steps, optimizer_ls, lr_scheduler_ls, num_lora)
                        # evaluate on the holdout set
                        if config.global_step_ls[0] % eval_step == 0:    
                            model = merge_lora_for_inference(model, num_lora, config.dpo_inference_name)
                    ...
                    for i in range(num_lora):
                        if config.global_step_ls[i] % save_step == 0:
                            model.save_pretrained(f'{prefix}/epoch_{epoch_index}_{mode}_{seed}_may_10_{config.lr}_{config.gamma}_resume_{resume}_{config.global_step_ls[i]}_replay_{replay_step}_offline_{offline_enable}_shuffle_{shuffle_data}_{rank_strategy}_on_policy_{on_policy_rand}_ret_{num_ret}_sample_{run_sample_strategy}_{sampling_update_step}_{train_baseline_model_mode}')
                            # save optimizer and lr scheduler
                            torch.save(optimizer_ls[i].state_dict(), f'{prefix}/epoch_{epoch_index}_{mode}_{seed}_may_10_{config.lr}_{config.gamma}_resume_{resume}_{config.global_step_ls[i]}_replay_{replay_step}_offline_{offline_enable}_shuffle_{shuffle_data}_{rank_strategy}_on_policy_{on_policy_rand}_ret_{num_ret}_sample_{run_sample_strategy}_{sampling_update_step}_{train_baseline_model_mode}/optimizer_{i}.pt')
                            torch.save(lr_scheduler_ls[i].state_dict(), f'{prefix}/epoch_{epoch_index}_{mode}_{seed}_may_10_{config.lr}_{config.gamma}_resume_{resume}_{config.global_step_ls[i]}_replay_{replay_step}_offline_{offline_enable}_shuffle_{shuffle_data}_{rank_strategy}_on_policy_{on_policy_rand}_ret_{num_ret}_sample_{run_sample_strategy}_{sampling_update_step}_{train_baseline_model_mode}/scheduler_{i}.pt')
                    pbar.update(1)
